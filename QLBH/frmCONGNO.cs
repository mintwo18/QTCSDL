...
namespace NhapHang
{
    ...
    {
        string connectionString = @"Data Source=PAOCHOUZ\BAOCHAU;Initial Catalog=QLBH;Integrated Security=True;Encrypt=True";
        SqlConnection conn;
        ...
        private void LoadData()
        {
            try
            {
                ...
                dataGridViewCONGNO.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                conn.Close();
            }
            catch (Exception ex)
            {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            txtSOTIENNO.Text = string.Format("{0:N0} VND", tienNo);
            txtSOTIENTRA.Text = string.Format("{0:N0} VND", tienTra);
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (index >= 0)
            {
                txtMACN.Text = dataGridViewCONGNO.Rows[index].Cells["MACN"].Value.ToString();
                txtMADDH.Text = dataGridViewCONGNO.Rows[index].Cells["MADDH"].Value.ToString();
 
                decimal tienNo = 0;
                if (decimal.TryParse(dataGridViewCONGNO.CurrentRow.Cells["SOTIEN_NO"].Value.ToString(), out tienNo))
                {
                    txtSOTIENNO.Text = tienNo.ToString("N0", new System.Globalization.CultureInfo("vi-VN")) + " VND";
                }
                decimal tienTra = 0;
                if (decimal.TryParse(dataGridViewCONGNO.CurrentRow.Cells["SOTIEN_TRA"].Value.ToString(), out tienTra))
                {
                    txtSOTIENTRA.Text = tienTra.ToString("N0", new System.Globalization.CultureInfo("vi-VN")) + " VND";
                }
